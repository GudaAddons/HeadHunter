...
return function(T, H)
    ...
    T.case("a catch pays by the outlaw's rank, once per outlaw per minute", function()
        ...
        T.ok(H.Printed("%+3|r mark%(s%): brought down .*Ganker.* Gank%-Stonespine %(3 marks%)"), "chat line")
    ...
    T.case("no marks for hunting 10+ levels down", function()
        ...
        T.ok(H.Printed("No marks: Lowbie%-Stonespine is 10%+ levels below you"), "said why")
    end)
    ...
    T.case("My marks tab and /hh marks", function()
        local ns = H.Boot({ client = "era" })
        Spree(ns, "Gank-Stonespine", 4)
        Settle()
        ns.Posse:Join(Entry(ns, "Gank-Stonespine"), { mapID = 1436, x = 0.5, y = 0.5 })
        ...
        H.Slash("marks")
        T.ok(H.Printed("HeadHunter rank: .*Tracker.* · 4 marks"), "status")
