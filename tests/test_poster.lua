...
return function(T, H)
    ...
    T.case("content: who, status, last kill, history, recent kills newest first", function()
        ...
        T.eq(c.where.mapID, 1436, "where: map")
    ...
    T.case("last kill button: waypoint, or coordinates where the client has none", function()
        local ns = H.Boot({ client = "era" })
        Spree(ns, "Gank-Stonespine", 4)
        Settle()
        ns.Poster:Show("Gank-Stonespine")
        T.eq(ns.Poster:GoToLastKill(), "waypoint", "waypoint")
        T.eq(H.waypoints[#H.waypoints].uiMapID, 1436, "on the kill's map")
        H.noWaypoints = true
        T.eq(ns.Poster:GoToLastKill(), "coords", "coordinates")
        T.ok(H.Printed("Gank%-Stonespine's last kill at 30%.0, 70%.0%."), "said where")
    end)
